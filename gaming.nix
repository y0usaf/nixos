...
#===============================================================================
{
  ...
  config = lib.mkIf globals.enableGaming {
    ...
    users.users.${globals.username}.extraGroups = ["gamemode"];

    # Gaming-related packages from home.nix
    ...
    # Steam directory configuration from globals.nix
    home.file = {
      ".local/share/Steam".source = lib.mkIf (globals.steamDir != null) (
        config.lib.file.mkOutOfStoreSymlink globals.steamDir
      );
      ...
      ".local/share/Steam/steamapps/compatdata/2767030/pfx/drive_c/users/steamuser/AppData/Local/Marvel/Saved/Config/Windows/GameUserSettings.ini" = {
        ...
        '';
        onChange = ''
          COMPATDATA="${config.home.homeDirectory}/.local/share/Steam/steamapps/compatdata/2767030"
          if [ -d "$COMPATDATA" ]; then
            DIR="$COMPATDATA/pfx/drive_c/users/steamuser/AppData/Local/Marvel/Saved/Config/Windows"
            mkdir -p "$DIR"
          fi
        '';
      };
