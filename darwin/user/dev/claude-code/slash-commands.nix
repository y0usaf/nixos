_: {
  home-manager.users.y0usaf = {
    home.file = {
      ...
      ".claude/commands/darwin-build.md" = {
        text = ''
          # /darwin-build
          Build and switch Darwin configuration safely:
          1. Format code with `alejandra .`
          2. Test build with `nh darwin switch --dry ~/nixos`
          3. Apply changes with `nh darwin switch ~/nixos`
