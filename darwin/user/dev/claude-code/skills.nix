_: {
  home-manager.users.y0usaf.home.file.".claude/skills/build-status-checker/SKILL.md" = {
    text = ''
      ---
      ...
      ## Purpose
      Autonomously verifies that `nh os switch` or `nh darwin switch` completed successfully before suggesting commits to git. This prevents committing broken configurations.

      ## When to Use
      After running:
      - `alejandra . && nh darwin switch ~/nixos` (Darwin)
      - `alejandra . && nh os switch --dry && nh os switch` (NixOS)
      ...
      - Verifies system configuration success
      - Confirms system is operational
      - Suggests safe commit message if everything passed
    '';
  };

  home-manager.users.y0usaf.home.file.".claude/skills/build-status-checker/check-build-status.sh" = {
    executable = true;
    text = ''
      #!/usr/bin/env bash

      # Build Status Checker - Verifies NixOS/Darwin rebuild success
      ...
      # Check for recent build errors in system logs
      if log show --predicate 'process == "nix"' --last 1h 2>/dev/null | grep -qi "error\|failed"; then
        echo "Recent build errors detected in system logs"
        exit 1
      fi

      # Verify system configuration is accessible
      if [[ ! -d "/run/current-system" ]] && [[ ! -L "/run/current-system" ]]; then
        if [[ ! -d "/nix/var/nix/profiles/system" ]]; then
          echo "⚠ Could not verify system configuration (may be normal on Darwin)"
        fi
      fi
  ...
  home-manager.users.y0usaf.home.file.".claude/skills/configuration-consistency-auditor/SKILL.md" = {
    text = ''
      ---
  ...
  home-manager.users.y0usaf.home.file.".claude/skills/configuration-consistency-auditor/audit-consistency.sh" = {
    executable = true;
    text = ''
      #!/usr/bin/env bash
  ...
  home-manager.users.y0usaf.home.file.".claude/skills/nix-security-scanner/SKILL.md" = {
    text = ''
      ---
  ...
  home-manager.users.y0usaf.home.file.".claude/skills/nix-security-scanner/scan-security.sh" = {
    executable = true;
    text = ''
      #!/usr/bin/env bash
