  ...
  netconsoleLoad = ''
    ...
    for _ in $(seq 1 30); do
      for path in /sys/class/net/*; do
        dev=$(basename "$path")
        [ "$dev" = lo ] && continue
        if modprobe netconsole "netconsole=+6665@${serverIp}/$dev,6666@192.168.2.28/58:11:22:b7:f0:29" 2>/dev/null; then
...
in {
  ...
  finit = {
    services.watchdog-keepalive = {
      ...
      command = "${pkgs.writeShellScript "watchdog-keepalive" ''
        set -u
