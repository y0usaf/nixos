...
in {
  ...
  config = lib.mkIf cfg.enable {
    ...
    users.users.y0usaf.maid.file.home."{{xdg_config_home}}/zsh/.zshrc".text = lib.mkAfter ''
      ...
      alias ytmp4s="yt-dlp --extractor-args 'youtube:player_client=android' -f 'bv*[height<=480]+ba/b[height<=480]' --recode-video mp4 --embed-metadata --add-metadata --postprocessor-args 'ffmpeg:-c:v libx264 -crf 26 -preset faster -c:a aac -b:a 96k -vf scale=-2:480' -o '%(title)s.%(ext)s'"
      alias ytwebm="yt-dlp --extractor-args 'youtube:player_client=android' -f 'bv*[height<=720]+ba/b[height<=720]' --recode-video webm --embed-metadata --add-metadata --postprocessor-args 'ffmpeg:-c:v libvpx-vp9 -crf 30 -b:v 0 -c:a libopus -vf scale=-2:720' -o '%(title)s.%(ext)s'"
      # Discord-friendly - 8MB limit version
      alias ytdiscord="yt-dlp --extractor-args 'youtube:player_client=android' -f 'bv*[height<=720]+ba/b[height<=720]' --recode-video mp4 --embed-metadata --add-metadata --postprocessor-args 'ffmpeg:-c:v libx264 -crf 28 -preset faster -c:a aac -b:a 96k -vf scale=-2:min(720\\,ih) -fs 7.8M' -o '%(title)s_discord.%(ext)s'"
    '';
  };
}
