...
# Extracted verbatim from the pre-restructure finix/default.nix.
{
  pkgs,
  lib,
  serverPersistent,
  ...
  #
  # Remote half; runs as root on the server under either OS. Manages a fully
  ...
  espIslandScript = pkgs.writeShellScript "finix-esp-island" ''
    ...
    do_install() {
      ...
      # Early microcode: the BIOS ships ancient ucode (0x1a vs 0x21) and every
      # kexec-era finix boot silently inherited the ucode NixOS loaded earlier
      # in the power cycle - but direct firmware boots ran the raw BIOS ucode,
      # and 2/2 of them misbehaved (00:19 early hang; 10:28Z hard freeze after
      # 5.5h, mid-heartbeat, zero kernel output, watchdog no-show). Prepend the
      # Intel early-ucode cpio exactly like NixOS does so direct boots match.
  ...
  # Desktop-side driver for the ESP island.
  bootDriverScript = pkgs.writeShellScriptBin "finix-server-boot" ''
    set -euo pipefail
    ...
    case "$action" in
      ...
      *)
        echo "usage: finix-server-boot [host] [status|bootnext-test|install|oneshot|promote|promote-force|demote|rollback]" >&2
        exit 2
        ;;
    esac
    ...
    # Tailscale SSH in check mode intercepts tailnet-resolved names (e.g. the
    # `server` alias via MagicDNS) and waits forever on a browser auth —
    # automation must fail fast instead of hanging. Target the LAN IP
    # (192.168.2.66) to use the classic sshd path; interception only happens
    # to traffic arriving over the tailnet.
    ...
    verify afterwards:  nix run .#finix-server-boot -- <host> status
    ...
    one-shot island boot initiated:
      - success: ssh y0usaf@192.168.2.66 comes back as finix (direct boot,
        no kexec); bootnext-deadman arms then clears BootNext automatically
      - kernel/initrd trouble: panic=30 + BootOrder fall back to NixOS
      - pre-OS hang (no panic): power-cycle -> NixOS (BootOrder is safe)
    then:  nix run .#finix-server-boot -- 192.168.2.66 promote
    MSG
        ;;
    esac
  '';
