{
  ...
  options.user.gaming.marvel-rivals.gameusersettings = {
    enable = lib.mkOption {
      type = lib.types.bool;
      default = false;
  ...
  config = lib.mkIf config.user.gaming.marvel-rivals.gameusersettings.enable {
    manzil.users."${config.user.name}".files."${lib.removePrefix "${config.user.homeDirectory}/" config.user.paths.steam.path}/steamapps/compatdata/2767030/pfx/drive_c/users/steamuser/AppData/Local/Marvel/Saved/Config/Windows/GameUserSettings.ini" = {
