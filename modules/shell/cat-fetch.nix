          ...
          text = ''
            print_cats() {
              ...
              if [ -f "$HOME/.cache/wallust/shell-colors.sh" ]; then
                ...
                local rudo_colour='\033[38;5;9m'     # Bright red
              fi
        ...
        ".config/nushell/cat-fetch.nu" = {
          text = ''
            def print_cats [] {
              ...
              let rudo = $"($e)[38;5;9m"    # Bright red
              let r = (ansi reset)

              let line1 = $"($tomoe) ⟋|､      ($moon)  ⟋|､      ($ekko)  ⟋|､      ($rudo)  ⟋|､"
              let line2 = [$tomoe "(°､ ｡ 7    " $moon "(°､ ｡ 7    " $ekko "(°､ ｡ 7    " $rudo "(°､ ｡ 7"] | str join
              let line3 = $"($tomoe) |､  ~ヽ   ($moon) |､  ~ヽ   ($ekko) |､  ~ヽ   ($rudo) |､  ~ヽ"
              let line4 = [$tomoe " じしf_,)〳" $moon " じしf_,)〳" $ekko " じしf_,)〳" $rudo " じしf_,)〳"] | str join
              let line5 = $"($rudo)  [tomo]   ($tomoe)  [moon]   ($moon)  [ekko]   ($ekko)  [rudo]($r)"
