{
  ...
  options.user.tools.npins-build = {
    enable = lib.mkEnableOption "npins build helpers and shell integration";
  };

  config = lib.mkIf config.user.tools.npins-build.enable {
        ...
        ".config/zsh/.zshrc" = {
          text = lib.mkAfter ''
            ...
            nbs() {
              ...
              nix-build -E "(import ./default.nix).nixosConfigurations.$(hostname).config.system.build.toplevel" $dry "$@"
            }
            ...
            nbswitch() {
              ...
              local result=$(nix-build -E "(import ./default.nix).nixosConfigurations.$(hostname).config.system.build.toplevel" --no-out-link)
              if [ $? -eq 0 ]; then
