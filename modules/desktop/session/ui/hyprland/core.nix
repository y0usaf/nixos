{
  ...
  cursorPackage = config.user.ui.cursor.package;
  hyprcursorThemeName = cursorPackage.hyprcursorThemeName or null;
  hasHyprcursor = hyprcursorThemeName != null;
  ...
  adaptiveHyprcursorLua = pkgs.writeText "hypr-adaptive-hyprcursor.lua" ''
    local config = ${lib.generators.toLua {} adaptiveHyprcursor}
  ...
  config = lib.mkIf config.user.ui.hyprland.enable {
    manzil.users."${config.user.name}".files.".config/hypr/hyprland.conf" = {
      text = lib.mkAfter ''
        ...
        ${lib.optionalString hasHyprcursor "exec-once = ${pkgs.lua5_4}/bin/lua ${adaptiveHyprcursorLua}"}
