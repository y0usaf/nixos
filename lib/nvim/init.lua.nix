# Neovim init.lua generator - shared across all systems
# Takes nvim lib config and generates the full init.lua content
{lib}: let
  nvim = import ./default.nix;
in
  # Remove init.lua.nix from the import to avoid circular reference
  let
    nvimExports = {
  ...
  in ''
    ...
    -- Load plugins
    vim.pack.add({
      ${lib.concatMapStringsSep ",\n        " (p: "\"${p}\"") nvimExports.plugins}
    })

    ${nvimExports.settings}
