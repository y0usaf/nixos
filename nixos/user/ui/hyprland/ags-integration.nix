  ...
  agsConfig = {
    "exec-once" =
      ...
      ++ [
        ...
        "for monitor in $(hyprctl monitors -j | jq -r '.[].name'); do wall=$(find ${config.user.directories.wallpapers.static.path} -type f | shuf -n 1); swaybg -o $monitor -i $wall -m fill & done"
      ];
